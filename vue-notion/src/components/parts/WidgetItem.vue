  ...
  <div class="widget-family">
    <div class="widget" @mouseover="onMouseOver" @mouseleave="onMouseLeave"
      ...
      <template v-if="widget.type == 'heading'">
        <input v-model="widget.text" class="heading transparent" placeholder="見出し" />
      </template>
      <template v-if="widget.type == 'body'">
        ・<input v-model="widget.text" class="body transparent" placeholder="本文" />
      </template>
      <template v-if="widget.type == 'code'">
        <textarea v-model="widget.text" class="code" rows="1" placeholder="コード" v-bind:ref="'widget-code-' + widget.id">
        </textarea>
      </template>
